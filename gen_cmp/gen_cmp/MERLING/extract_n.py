...
class gen_cmp():
    ...
    def make_input_output_file_list (self, rootpath):
        if not os.path.exists(rootpath + '/cmp'):
            os.mkdir(rootpath+'/cmp')
            os.mkdir(rootpath + '/normed_cmp')
        file_ids = os.listdir (rootpath + '/lf0')
        file_ids = [item.split ('.')[0] for item in file_ids]

        file_checked_ids = []

        for item in file_ids:
            lf0_length = np.fromfile (rootpath + '/lf0/' + item + '.lf0', dtype=np.float32).reshape ((-1, 1)).shape[0]
            bap_length = np.fromfile (rootpath + '/bap/' + item + '.bap', dtype=np.float32).reshape ((-1, 1)).shape[0]
            mgc_length = np.fromfile (rootpath + '/mgc/' + item + '.mgc', dtype=np.float32).reshape ((-1, 60)).shape[0]
        ...
        for name in ['lf0', 'bap', 'mgc']:
            ...
            value = [rootpath + '/' + name + '/' + item + '.' + name for item in file_checked_ids]
            input_dict.update ({name : value})

        output_list = [rootpath  + '/cmp/' + item + '.cmp' for item in file_checked_ids]

        output_norm_list = [rootpath + '/normed_cmp/' + item + '.cmp' for item in file_checked_ids]
        return input_dict, output_list, output_norm_list

    def get_normed_cmp(self, rootpath):
        delta_win = [-0.5, 0.0, 0.5]
        acc_win = [1.0, -2.0, 1.0]

        acoustic_worker = AcousticComposition(delta_win=delta_win, acc_win=acc_win)

        in_file_list_dict, nn_cmp_file_list, nn_cmp_norm_file_list = self.make_input_output_file_list(rootpath)
        ...
        #var_dir = os.path.join(rootpath, '/var/')
        ...
        for feature_name in out_dimension_dict.keys():
            var_file_dict[feature_name] = os.path.join(var_dir,
                                                       feature_name + '_' + str(out_dimension_dict[feature_name]))
        ...
        norm_info_file = rootpath + '/norm_info_file'
        ...
        for feature_name in out_dimension_dict.keys():
            feature_std_vector = np.array(
                global_std_vector[:, feature_index:feature_index + out_dimension_dict[feature_name]], 'float32')
